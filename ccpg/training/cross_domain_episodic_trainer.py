...
from ccpg.ccsa import select_episode_category_semantics
from ccpg.cra import CategoryRelationalAlignmentLoss
from ccpg.data import (
...
from ccpg.evaluation import evaluate_target_domain
from ccpg.lcp import (
    build_class_identity_vectors,
    generate_label_conditioned_priors,
)
from ccpg.runtime import (
...
from ccpg.training.model_factory import CCPGModelBundle, build_ccpg_models
from ccpg.training.optimizer_factory import (
    build_ccpg_optimizers,
    step_optimizers,
    zero_optimizer_gradients,
)
from ccpg.training.prototypical_classification import (
    average_support_features,
    l2_normalize,
    negative_squared_euclidean_logits,
)
from ccpg.visualization import save_classification_map


def load_config(config_path: str):
    spec = importlib.util.spec_from_file_location(
        "ccpg_experiment_config",
        config_path,
    )
...
def build_argument_parser(
    ...
    parser = argparse.ArgumentParser(
        description="Category-Conditioned Prior Guidance (CCPG) training."
    )
    parser.add_argument(
        "--config",
        type=str,
...
def run_ccpg_training(
    ...
    logger = logging.getLogger("ccpg")
    logger.info("method_name: CCPG")
    ...
    for run_index, seed in enumerate(seeds):
        ...
        optimizers = build_ccpg_optimizers(
            models,
